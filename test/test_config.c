...
{
    int argc = 8;
    char *argv[8] = {"cerids", "-d", "-i", "eth0", "-f" , "test", "-p", "port 80"};
    Options *options = malloc(sizeof(Options));
    optind = 0;

    CU_ASSERT(getConfByArgs(argc, argv, options) == 0);
    CU_ASSERT(options->debug && options->foreground && strcmp(options->dev, "eth0") == 0 && options->filename != NULL && strcmp(options->filter, "port 80") == 0);
